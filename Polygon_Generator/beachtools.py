...
def read_gjsons(smaller, larger):
    ...
    for json in [smaller, larger]:
        os.system('ogr2ogr -nlt LINESTRING -skipfailures ' +
                  json[:-8] + '.shp ' + json + ' OGRGeoJSON')
...
def check_for_intersects(smaller,larger):
    print(smaller.intersection(larger))
...
def make_and_write_ta(merged, out_name):
    ...
    ds=outdriver.CreateDataSource(out_name)
    # ds = driver.CreateDataSource(out_name)
    tmp=outdriver.Open(out_name)
